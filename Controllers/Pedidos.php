...
class Pedidos extends Controllers
{
            ...
            for ($i = 0; $i < count($arrData); $i++) {
                ...
                if ($_SESSION['permisosMod']['r']) {
                    ...
                    if ($arrData[$i]['idtipopago'] == 1) {
                        $btnView .= ' <a title="Ver transaccion" href="' . base_url() . '/pedidos/transaccion/' . $arrData[$i]['idtransaccionpaypal'] . '" target="_blanck"
                        class="btn btn-info btn-sm"><i class="fa fa-paypal" 
    ...
    public function orden($idpedido)
    {
        ...
        $data['page_tag'] = "Pedido - Oh my Pet";
        $data['page_title'] = "PEDIDOS <small>Oh my Pet</small>";
    ...
    public function transaccion($transaccion)
    {
        ...
        $data['page_tag'] = "Detalles de la transaccion - Oh my Pet";
        ...
        if ($_POST) {
            if ($_SESSION['permisosMod']['u'] and $_SESSION['userData']['idrol'] != RCLIENTES) {
                ...
                if ($idpedido == "") {
                    ...
                    if ($idtipopago == "") {
                        if ($estado == "") {
                            $arrResponse = array("status" => false, "msg" => 'Datos incorrectos2.');
                        } else {
                        ...
                        if ($transaccion == "" or $idtipopago == "" or $estado == "") {
                            $arrResponse = array("status" => false, "msg" => "Datos incorrectos3.");
                        } else {
