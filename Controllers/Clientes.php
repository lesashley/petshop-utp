...
class Clientes extends Controllers{
    public function __construct()
    {
        ...
        getPermisos(3);  //para que solo puedan entrar los usuarios con permisos 3 (modulos clientes)
    }
    ...
    public function Clientes()
    {
        ...
        $data['page_title'] = '<i class="fas fa-user-tag"></i>'." CLIENTES ";
    ...
    public function setCliente(){
		...
		if($_POST){
			if(empty($_POST['txtIdentificacion']) || empty($_POST['txtNombre']) || empty($_POST['txtApellido']) || empty($_POST['txtTelefono']) || empty($_POST['txtEmail']) || empty($_POST['txtNit']) || empty($_POST['txtNombreFiscal']) || empty($_POST['txtDirFiscal']) )
			{
				...
				{
					$option = 1;
					$strPassword =  empty($_POST['txtPassword']) ? passGenerator() : $_POST['txtPassword'];//sin encriptar para el correo del cliente
