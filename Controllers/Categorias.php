<?php
	class Categorias extends Controllers{
		...
		public function Categorias()
		{
			...
			$data['page_title'] = '<i class="fas fa-user-tag"></i>'." CATEGORIAS";
		...
		public function setCategoria(){
			if($_POST){
				if(empty($_POST['txtNombre']) || empty($_POST['txtDescripcion']) || empty($_POST['listStatus']) )
				{
					...
					$ruta=strtolower(clear_cadena($strCategoria)); //rutas amigables,limpia la cadena
			...
			if($_SESSION['permisosMod']['r']){
				...
				if($intIdcategoria > 0)
				{
					...
					if(empty($arrData))
					{
						...
						$arrData['url_portada'] = media().'/images/uploads/'.$arrData['portada']; //ruta completa de la imagen
